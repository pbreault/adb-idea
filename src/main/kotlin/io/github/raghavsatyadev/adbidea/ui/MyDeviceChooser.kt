...
class MyDeviceChooser(
  ...
  @Volatile private var myProcessSelectionFlag = true
   ...
   * thread. If it is different than [.myDisplayedDevices], then a [.refreshTable] invocation in the
   * EDT thread will update the displayed list to match the detected list.
   */
    ...
    get() {
      ...
      for (row in rows) {
        if (row >= 0) {
          ...
          val bridge =
            AndroidSdkUtils.getDebugBridge(myFacet.module.project) ?: return EMPTY_DEVICE_ARRAY
  ...
  private inner class MyDeviceTableModel(val myDevices: Array<IDevice>) : AbstractTableModel() {
    ...
    override fun getValueAt(rowIndex: Int, columnIndex: Int): Any? {
      ...
      when (columnIndex) {
        ...
        COMPATIBILITY_COLUMN_INDEX -> {
          ...
          return try {
            if (myMinSdkVersion.isDone)
              connectedDevice.canRun(
                myMinSdkVersion.get(),
                myProjectTarget,
    ...
    private fun androidDevice(device: IDevice) =
      object : BackwardCompatibleGetter<AndroidDevice>() {
        ...
        override fun getPreviousImplementation(): AndroidDevice {
          return Reflect.onClass("com.android.tools.idea.run.ConnectedAndroidDevice")
            .create(device)
            .get()
        }
      }
  ...
  companion object {
    ...
    private fun getRequiredHardwareFeatures(
      requiredFeatures: List<UsesFeature>
    ): EnumSet<
      HardwareFeature
    > { // Currently, this method is hardcoded to only search if the list of required features
        // includes a watch.
    ...
    private fun getDeviceState(device: IDevice): String {
      ...
      return if (state != null) StringUtil.capitalize(state.name.lowercase(Locale.getDefault()))
      else ""
    }
  }
  ...
  init {
    ...
    myDeviceTable.setSelectionMode(
      if (multipleSelection) ListSelectionModel.MULTIPLE_INTERVAL_SELECTION
    ...
    setColumnWidth(
      myDeviceTable,
      DEVICE_NAME_COLUMN_INDEX,
      "Samsung Galaxy Nexus Android 4.1 (API 17)",
    )
