      ...
      try {
        if (deviceHasMarshmallow(device))
          if (AdbUtil.isAppInstalled(device, packageName)) {
            ...
            shellOutputReceiver.adbOutputLines
              .stream() // only granted permissions, they come in "android.permission.CAMERA:
                // granted=true"
              .filter { s: String -> s.contains("permission") }
              .filter { s: String ->
                s.contains("granted=true")
              } // just the permission name is important
              .map { s: String -> s.split(":".toRegex()).toTypedArray()[0].trim { it <= ' ' } }
              .forEach { s: String ->
                try {
                  device.executeShellCommand(
                    "pm revoke $packageName $s",
                    GenericReceiver(),
                    15L,
                    TimeUnit.SECONDS,
                  )
                  NotificationHelper.info(
                    String.format("Permission <b>%s</b> revoked on %s", s, device.name)
                  )
                } catch (e: Exception) {
