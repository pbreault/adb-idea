      ...
      try {
        if (deviceHasMarshmallow(device))
          if (AdbUtil.isAppInstalled(device, packageName)) {
            ...
            adbOutputLines.forEach(
              Consumer { s: String ->
                try {
                  device.executeShellCommand(
                    "pm grant $packageName $s",
                    GenericReceiver(),
                    15L,
                    TimeUnit.SECONDS,
                  )
                  NotificationHelper.info(
                    String.format("Permission <b>%s</b> granted on %s", s, device.name)
                  )
                } catch (e: Exception) {
                  NotificationHelper.error(
                    String.format("Granting %s failed on %s: %s", s, device.name, e.message)
                  )
                }
              }
            )
