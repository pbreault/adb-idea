...
import io.github.raghavsatyadev.adbidea.ObjectGraph
import io.github.raghavsatyadev.adbidea.adb.DeviceResult.SuccessfulDeviceResult
import io.github.raghavsatyadev.adbidea.adb.command.*
import io.github.raghavsatyadev.adbidea.adb.command.SvcCommand.MOBILE
import io.github.raghavsatyadev.adbidea.adb.command.SvcCommand.WIFI
import io.github.raghavsatyadev.adbidea.ui.NotificationHelper
import java.util.concurrent.Executors

object AdbFacade {
  private val EXECUTOR =
    Executors.newCachedThreadPool(ThreadFactoryBuilder().setNameFormat("AdbIdea-%d").build())
  ...
  private fun executeOnDevice(project: Project, runnable: Command) {
    ...
    when (val result = objectGraph.deviceResultFetcher.fetch()) {
      ...
      is DeviceResult.DeviceNotFound,
      null -> NotificationHelper.error("No device found")
    }
  }
}
